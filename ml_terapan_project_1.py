...
Tujuan utama proyek ini adalah untuk memprediksi harga saham Microsoft dengan akurat. Berdasarkan hasil evaluasi, model Linear Regression telah berhasil mencapai tujuan ini dengan sangat baik dari segi prediksi nilai (magnitude), namun gagal dalam memprediksi arah pergerakan.

**Implikasi Praktis:**
1. **Aplikasi Manajemen Portofolio**: Model ini sangat berguna untuk valuasi aset dan proyeksi nilai portofolio karena akurasi prediksi nilai yang tinggi (MAPE 1.09%).

2. **Keterbatasan untuk Trading Aktif**: Dengan directional accuracy di bawah 50%, model ini tidak disarankan untuk strategi trading yang mengandalkan prediksi arah pergerakan harga.

3. **Periode Volatilitas Tinggi**: Error meningkat selama periode volatilitas ekstrem seperti krisis COVID-19, menunjukkan perlunya kehati-hatian saat menggunakan model pada kondisi pasar tidak normal.

### Diskusi tentang Metrik

Pemilihan metrik evaluasi dalam proyek ini sangat sesuai dengan konteks prediksi harga saham:

1. **RMSE vs MAE**: RMSE (0.0110) lebih sensitif terhadap error besar, menunjukkan bahwa model jarang membuat kesalahan signifikan dalam prediksinya.

2. **MAPE untuk Perspektif Investasi**: MAPE (1.09%) memberikan gambaran persentase error, yang lebih sesuai dengan cara investor memandang pergerakan harga.

3. **R² untuk Kekuatan Model**: R² yang sangat tinggi (0.9973) mengkonfirmasi bahwa model Linear Regression sangat cocok untuk data saham Microsoft, meskipun sangat sederhana strukturnya.

4. **Directional Accuracy untuk Trading**: Metrik ini mengungkap keterbatasan utama model - ketidakmampuannya memprediksi arah pergerakan dengan akurasi lebih baik dari tebakan acak.

### Kesimpulan Evaluasi

Model Linear Regression menunjukkan performa excellent dalam memprediksi nilai harga saham Microsoft, dengan error persentase rata-rata hanya 1.09%. Namun, model gagal memprediksi arah pergerakan harga secara akurat.

Temuan ini konsisten dengan Efficient Market Hypothesis - bahwa harga saat ini adalah prediktor terbaik untuk harga masa depan dalam pasar yang efisien. Koefisien 0.9993 pada model Linear Regression mengkonfirmasi bahwa prediksi terbaik untuk harga Microsoft besok adalah harga hari ini, dengan penyesuaian minimal.

Untuk meningkatkan kemampuan model dalam memprediksi arah pergerakan, direkomendasikan untuk mengeksplorasi fitur eksternal yang tidak terkandung dalam harga historis, seperti sentimen pasar, data fundamental, atau indikator makroekonomi.
"""