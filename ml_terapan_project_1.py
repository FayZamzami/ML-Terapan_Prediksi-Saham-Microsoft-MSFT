...
- Ditemukan 1 missing value pada kolom 'dtype' (int64)

- Tidak ada missing values pada kolom-kolom utama (open, high, low, close,
adj_close, volume)
...
Sebagai langkah final dalam preprocessing, dilakukan pengecekan dan penanganan missing values dengan dropna(). Missing values terutama muncul dari operasi lag dan rolling windows dalam pembuatan technical indicators. Penghapusan baris dengan missing values adalah pendekatan yang aman dalam kasus ini karena jumlah data yang hilang relatif kecil dibanding ukuran dataset total, dan interpolasi bisa berisiko dalam konteks prediksi time series.
"""
...
"""##### **Tidak ditemukan data duplikat** dalam dataset. Setiap baris merepresentasikan hari trading yang unik
...
**Konversi Date dan Pembuatan Fitur Temporal**

Dalam tahap persiapan data, salah satu langkah krusial yang dilakukan adalah konversi kolom 'date' dari tipe data string/object menjadi datetime64[ns]. Konversi ini dilakukan menggunakan fungsi pd.to_datetime() yang secara otomatis mendeteksi format tanggal umum (YYYY-MM-DD). Transformasi ini sangat penting karena memungkinkan berbagai operasi manipulasi tanggal yang diperlukan untuk analisis time series, seperti perhitungan selisih hari, pengurutan kronologis, dan yang terpenting, ekstraksi komponen waktu yang berbeda.

Setelah konversi ke datetime, dilakukan ekstraksi berbagai komponen temporal untuk memperkaya dataset dengan informasi waktu yang lebih granular. Dari kolom datetime, diekstrak enam fitur temporal utama: tahun (year), bulan (month), nama hari (day_of_week), kuartal (quarter), penanda awal bulan (is_month_start), dan penanda akhir bulan (is_month_end). Setiap fitur ini memiliki peran spesifik dalam analisis. Tahun (range 1986-2020) membantu menangkap tren jangka panjang dan siklus ekonomi. Bulan (1-12) dan kuartal (1-4) penting untuk mengidentifikasi pola musiman dan efek laporan keuangan kuartalan. Nama hari (Monday-Friday) memungkinkan analisis pola perdagangan harian, sementara penanda awal dan akhir bulan (True/False) membantu mendeteksi efek kalender seperti window dressing dan perilaku perdagangan di sekitar pergantian bulan.

Hasil ekstraksi fitur temporal ini menunjukkan beberapa pola menarik. Distribusi data per tahun relatif merata dengan sekitar 240-260 data per tahun, kecuali untuk tahun 2020 yang memiliki data lebih sedikit karena belum lengkap. Distribusi bulanan juga cukup seimbang, meski ada sedikit peningkatan di bulan Maret, Agustus, dan Oktober. Untuk hari perdagangan, terdapat distribusi yang relatif merata antara Senin hingga Jumat, dengan Selasa dan Rabu menunjukkan frekuensi sedikit lebih tinggi. Data kuartalan terdistribusi hampir sama untuk Q1 hingga Q4, menandakan tidak ada bias musiman yang signifikan. Untuk penanda awal dan akhir bulan, distribusinya sesuai ekspektasi dimana hanya sebagian kecil data yang merupakan hari pertama atau terakhir bulan, mencerminkan pola kalender yang natural.

Penambahan fitur temporal ini memperkaya dimensi analisis dataset, memungkinkan model untuk mengenali dan mempelajari berbagai pola temporal dalam pergerakan harga saham Microsoft. Fitur-fitur ini juga menjadi dasar untuk pengembangan indikator teknikal yang lebih kompleks dan dapat membantu dalam pemahaman efek kalender pada pasar saham. Keberagaman fitur temporal ini memberikan perspektif yang lebih komprehensif tentang bagaimana waktu mempengaruhi dinamika harga saham Microsoft.
...
Dalam proses persiapan data, dilakukan kategorisasi harga saham (price binning) melalui dua pendekatan: price_range dan price_range_numeric. Untuk price_range, harga saham dibagi menjadi enam kategori dengan interval $50 ($0-$50, $50-$100, dst. hingga $250-$300) menggunakan pd.cut(). Kategorisasi ini menghasilkan label kategorikal yang mudah diinterpretasi ('0-50', '50-100', dst.). Sementara itu, price_range_numeric menggunakan binning yang sama namun menghasilkan label numerik (0, 1, 2, dst.) dengan parameter labels=False. Tujuan dari binning ganda ini adalah untuk memberikan fleksibilitas dalam analisis: versi kategorikal berguna untuk visualisasi dan analisis deskriptif, sedangkan versi numerik lebih sesuai untuk pemodelan machine learning yang membutuhkan input numerik.
...
Berdasarkan analisis boxplot:

- Open: 1275 outliers terdeteksi
- High: 1276 outliers terdeteksi
- Low: 1276 outliers terdeteksi
- Close: 1278 outliers terdeteksi
- Adj_Close: 1324 outliers terdeteksi
...
Multikolinearitas diatasi menggunakan pendekatan sistematis dengan fungsi remove_collinear(). Fungsi ini menghitung matriks korelasi absolut antar fitur dan menggunakan threshold 0.85 sebagai batas untuk menentukan multikolinearitas. Proses ini menggunakan np.triu() untuk mengambil matriks segitiga atas, menghindari perhitungan redundan dari korelasi yang simetris. Dari 15 fitur teratas berdasarkan korelasi dengan target, 13 fitur teridentifikasi memiliki korelasi tinggi (>0.85) dan dieliminasi. Keputusan untuk mempertahankan hanya fitur 'close' didasarkan pada prinsip parsimony, mengingat fitur ini memiliki korelasi tertinggi dengan target dan fitur lainnya dapat dianggap sebagai transformasi dari 'close'. Pendekatan ini secara signifikan mengurangi dimensionalitas data tanpa kehilangan kemampuan prediktif yang substansial.
"""
...
Proses normalisasi menggunakan MinMaxScaler diterapkan secara terpisah pada fitur input (X) dan target (y). Scaler memastikan semua nilai berada dalam rentang [0,1], menghindari masalah skala dalam proses pembelajaran model. Dua scaler terpisah digunakan: satu untuk fitur input dan satu khusus untuk target (target_scaler). Target_scaler disimpan untuk keperluan denormalisasi prediksi nanti, memungkinkan evaluasi performa dalam skala harga asli. Normalisasi ini penting terutama karena range harga saham Microsoft yang lebar ($0.07 hingga $349.67) dapat menyebabkan masalah numerik dalam proses pembelajaran model.

**One-Hot Encoding :**

Untuk fitur kategorikal yang tersisa setelah seleksi fitur, diterapkan one-hot encoding menggunakan pd.get_dummies() dengan opsi drop_first=True untuk menghindari multikolinearitas dalam variabel dummy. Kolom boolean seperti is_month_end dan is_month_start dikonversi ke integer (0/1) untuk memastikan kompatibilitas dengan algoritma machine learning. Proses ini menghasilkan representasi numerik yang sesuai untuk semua fitur kategorikal sambil mempertahankan informasi kategorikal yang penting.
...
Pembagian data mengikuti prinsip time series dengan rasio 70:15:15 untuk training, validation, dan testing. Berbeda dengan split acak yang umum dalam machine learning, pembagian ini mempertahankan urutan kronologis data, yang krusial untuk prediksi time series. Training set (70%, periode 1988-2008) mencakup berbagai siklus pasar termasuk dot-com bubble, memberikan model eksposur terhadap berbagai kondisi pasar. Validation set (15%, 2008-2014) menangkap periode krisis finansial dan pemulihannya, ideal untuk menguji ketahanan model. Test set (15%, 2014-2020) mencakup periode bull market yang berkelanjutan dan volatilitas COVID-19, memberikan evaluasi yang realistis terhadap performa model dalam kondisi pasar terkini.
...
1. **Persiapan Data**
   ...
   - Pembuatan sequence data untuk model LSTM

2. **Implementasi Model**
   - Training 5 model berbeda
...
```
**Parameter:**
- Tidak memiliki parameter yang perlu di-tune
- Menggunakan implementasi default scikit-learn
- Cocok untuk hubungan linier antara fitur dan target
...
- n_estimators: Jumlah pohon dalam forest
  - Default: 100
  - Fungsi: Menentukan ukuran ensemble
- max_depth: Kedalaman maksimum tiap pohon
  - Default: None (ekspansi hingga leaf pure)
  - Fungsi: Kontrol kompleksitas model
- min_samples_split: Minimum sampel untuk split
  - Default: 2
  - Fungsi: Mencegah overfitting pada leaf nodes
...
- learning_rate: Step size shrinkage
  - Default: 0.1
  - Fungsi: Mencegah overfitting
- max_depth: Kedalaman maksimum pohon
  - Default: 3
  - Fungsi: Kontrol kompleksitas model
- subsample: Fraksi sampel untuk training
  - Default: 1
  - Fungsi: Mencegah overfitting
...
- n_estimators: Jumlah boosting stages
  - Default: 100
  - Fungsi: Menentukan jumlah iterasi
- learning_rate: Kontribusi setiap pohon
  - Default: 0.1
  - Fungsi: Mengontrol overfitting
...
```
**Parameter dan Fungsinya:**
- units: Dimensi output space
  - Value: 50
  - Fungsi: Kapasitas model
- dropout: Rate dropout
  - Value: 0.2
  - Fungsi: Regularisasi
- sequence_length: Panjang sequence
  - Value: 10
  - Fungsi: Konteks temporal
...
- max_depth: 5
- n_estimators: 300
- Improvement: RMSE turun 5%

[Bagian selanjutnya sama seperti yang sudah ada sebelumnya, mulai dari "Model Selection" hingga "Kesimpulan"]
...
Berikut hasil evaluasi model Linear Regression (model terbaik) pada data testing. Serta tidak memerlukan tuning karena tidak memiliki hyperparameter:
...
#### A. Linear Regression
Linear Regression tidak memerlukan tuning karena tidak memiliki hyperparameter.

**Hasil Evaluasi:**
| Metrik | Nilai | Interpretasi |
|--------|-------|--------------|
| RMSE | 0.0110 | Error kuadrat rata-rata sangat rendah |
| MAE | 0.0058 | Model rata-rata meleset 0.58% |
| MAPE | 1.09% | Error relatif sangat kecil |
| R² | 0.9973 | Model menjelaskan 99.73% variasi |
| Directional Accuracy | 47.44% | Tidak lebih baik dari random |
...
| MAPE | 1.43% | ↑ 5.9% |
| R² | 0.9912 | ↑ 0.1% |
| Directional Accuracy | 50.12% | ↑ 0.8% |
...
| MAPE | 1.15% | ↑ 4.2% |
| R² | 0.9943 | ↑ 0.05% |
| Directional Accuracy | 49.89% | ↑ 0.6% |
